...
registerBlockType("author-avatars/show-avatar", {
	...
	edit: withSelect((select) => {
		return {
			data: wp
				.apiFetch({ path: "/author_avatar/blocks/v1/data" })
				.then((data) => {
					// console.log(data);
		...
		const { className, user, data, isSelected, attributes, setAttributes } =
			props;
				...
				{display_options?.map((v) => (
					<li key={v.value}>
						<CheckboxControl
							...
							onChange={(check) => {
								check
									? (checked_obj[v.value] = true)
									: delete checked_obj[v.value];
		...
		return [
			<InspectorControls key={"000"}>
				<div className="author-avatar-components-panel">
					<SelectControl
						label={__(
							"User or Email addrerss/user_id or Roles",
							"author-avatar"
						)}
						name="user_id"
						value={user_id}
					...
					{0 == user_id && (
						<Fragment>
							<label className="blocks-base-control__label">
								{__("Which Roles to display:", "author-avatar")}
							</label>
					...
					<label className="blocks-base-control__label">
						{__("Info to show with avatar:", "author-avatar")}
					</label>
					<DisplayCheckBoxes />
					...
					<label className="blocks-base-control__label">
						{__("Background color", "author-avatar")}
					</label>
					...
					<Fragment>
						<a
							className={"donate"}
							href={
								"https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=MZTZ5S8MGF75C&lc=CA&item_name=Author%20Avatars%20Plugin%20Support&item_number=authoravatars&currency_code=CAD&bn=PP%2dDonationsBF%3abtn_donateCC_LG%2egif%3aNonHosted"
							}
							target={"_donante"}
						>
							<img
				...
				{0 == user_id && (
					<Fragment>
						...
						<TextareaControl
							...
							help={__(
								"(Comma separate list of user login ids. Hidden user are removed before the white list)",
								"author-avatar"
							)}
							value={hidden_users}
						...
						<TextareaControl
							...
							help={__(
								"(0nly show these users, Comma separate list of user login ids)",
								"author-avatar"
							)}
